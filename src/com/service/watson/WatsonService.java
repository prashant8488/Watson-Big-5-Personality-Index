					...
					JSONArray arr2=arr1.getJSONObject(j).getJSONArray("children");
						for(int k=0;k<arr2.length();k++){
								if(a<=5){
									...
									result.put(arr2.getJSONObject(k).getString("id")+"_SE",(int)Math.round(arr2.getJSONObject(k).getDouble("sampling_error")*100));
									
									for(int l=0;l<arr3.length();l++){
										result.put(arr3.getJSONObject(l).getString("id"),(int)Math.round(arr3.getJSONObject(l).getDouble("percentage")*100));
										result.put(arr3.getJSONObject(l).getString("id")+"_SE",(int)Math.round(arr3.getJSONObject(l).getDouble("sampling_error")*100));
